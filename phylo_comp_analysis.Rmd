...
fas_msa <-  msa(file, method = c("Muscle"), cluster = "default", type = "protein", verbose = TRUE, maxiters = 16)
print(fsa_msa, showConsensus = TRUE, show = "complete")
```
...
In order to to make a phylogenetic tree with our data, we need to calculate the number of AA differences between each pair of sequences in a matrix. This function can only be run on data in an "AAbin". Within this function are two arguments:\
...
```{r}
ggt <-ggtree(tree, cex = 0, aes(color = branch.length), root.position = 0) +
